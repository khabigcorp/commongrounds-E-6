    ...
    def get_context_data(self, **kwargs):
        ...
        if user.is_authenticated:
            ...
            signed_up = Event.objects.filter(
                signups__user_registrant=user_profile
            ).distinct()
...
class EventDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if user.is_authenticated:
            ...
            user_signed_up = event.signups.filter(
    ...
    def post(self, request, *args, **kwargs):
        ...
        if EventSignup.objects.filter(
            event=event, user_registrant=profile
        ).exists():
            return redirect("localevents:event_detail", pk=event.pk)
...
class EventSignupView(View):
    def get(self, request, pk):
        ...
        return render(
            request, "localevents/event_signup.html", {"event": event}
        )
    ...
    def post(self, request, pk):
        ...
        if not name:
            return render(
                request,
                "localevents/event_signup.html",
                {"event": event, "error": "Name is required."},
            )
        if event.signups.count() >= event.event_capacity:
            return render(
                request,
                "localevents/event_signup.html",
                {"event": event, "error": "Event is already full."},
            )
        if EventSignup.objects.filter(
            event=event, new_registrant=name
        ).exists():
            return render(
                request,
                "localevents/event_signup.html",
                {"event": event, "error": "You have already signed up."},
            )
